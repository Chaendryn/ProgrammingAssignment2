...
makeCacheMatrix <- function(myMatrix = matrix()) { 
      ...
      set <- function(y) { ## creates the 'set' accessor function, which sets the value of the matrix
            myMatrix <<- y ## Makes the 'set' function available to the other functions outside the environment of this function 
      ...
      setIMatrix <- function(solve) m <<- solve ## Sets the inverse matrix m, and makes the solve function available outside the scope of this function
      getIMatrix <- function() m ## gets the value of the inverse matrix
      list(set = set, get = get, ## creats the function list with get, set, setIMatrix and getIMatrix
...
cacheSolve <- function(myMatrix, ...) {
      ...
      data <- myMatrix$get() ## retrieves the variables for the matrix with which the solve function will calculate the inverse
